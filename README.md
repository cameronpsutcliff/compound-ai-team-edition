...
The installer uses these to place the kit and create your Teams content area. To declare what your machine can do (vector search, scheduled runs), edit `capability-profile.md` by hand.

**Step 3: Point your AI at the folder and start pasting intake.**

Open your AI (Claude, GPT, Gemini, anything). Give it the contents of `skills/team-router/SKILL.md` as context. Paste raw intake: emails, Slack threads, meeting notes, doc summaries. The router does the rest.
...
team-edition/
  ...
  bin/verify-portable.sh                 <- portability checker (run before sharing)
  skills/team-router/SKILL.md            <- the routing skill (paste into your AI)
  skills/team-router/templates/
...
Edit `skills/team-router/templates/routing-registry.md` to rename, add, or remove workstreams. The router adapts automatically.

---

## Capability Profile

`capability-profile.md` is the portability seam. It tells the router what this machine can do.
...
Before sharing this kit or committing it to a team repo, run:

```bash
bash bin/verify-portable.sh
```

The checker flags personal paths, vendor leakage, em-dashes, and a short list of banned filler words. Fix everything it flags before the kit leaves your machine.

---

## Credits

The three-layer "AI operating system" model and the A.C.E. structure are from Nick Milo (Linking Your Thinking). This kit is Compound AI by Cameron Sutcliff.
